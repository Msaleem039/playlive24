...
import { useGetSoccerMatchesQuery, useGetTabBannersQuery, useToggleMatchVisibilityMutation } from '@/app/services/Api'
...
export default function SoccerTab() {
  ...
  const { data: matchesData, isLoading, isError, refetch } = useGetSoccerMatchesQuery(undefined, {
    refetchOnMountOrArgChange: true,
  })
  ...
  const handleMatchSelect = (eventId: string, marketId?: string) => {
    ...
    if (isAgent) {
      ...
      router.push(`/live/${eventId}?sport=soccer${marketQuery}`)
    }
  }
  ...
  return (
    <div className="bg-white">
      ...
      <div className="divide-y divide-gray-200">
        {isLoading && <div className="text-xs text-gray-600 px-3 py-4">Loading soccer matches...</div>}
        {isError && <div className="text-xs text-red-600 px-3 py-4">Failed to load soccer matches.</div>}
        {!isLoading && !isError && events.length === 0 && <div className="text-xs text-gray-500 px-3 py-4">No soccer matches available.</div>}
        ...
        {!isLoading && !isError && events.map((item: any, index: number) => {
          ...
          const isBlocked =
            typeof item?.isMatchOddsBlocked === 'boolean'
              ? item.isMatchOddsBlocked
              : typeof item?.isMatchOddsAllowed === 'boolean'
              ? !item.isMatchOddsAllowed
              : item?.blocked === true || item?.isBlocked === true
          const effectiveBlocked =
            typeof matchBlockedOverrides[String(eventId)] === 'boolean'
              ? matchBlockedOverrides[String(eventId)]
              : isBlocked
                  ...
                  <div className="pt-1">
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault()
                        e.stopPropagation()
                        handleToggleMatchBlocked(String(eventId), effectiveBlocked)
                      }}
