        ...
        if (settlement.bets && Array.isArray(settlement.bets)) {
          settlement.bets.forEach((bet: any) => {
            historyBets.push({
              ...
              winnerId: settlement.winnerId ?? settlement.winner_id
            })
          })
        }
        // Include settlements with no bets so they still show (with winnerId)
        if (!settlement.bets?.length) {
          historyBets.push({
            id: settlement.id,
            settlementId: settlement.settlementId || settlement.id,
            settlement_id: settlement.settlementId || settlement.id,
            ...
            betName: settlement.marketType || "—",
            amount: settlement.statistics?.totalStake ?? 0,
            odds: "—",
            status: "—"
          })
        }
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-4 xl:px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase">Winner ID</th>
