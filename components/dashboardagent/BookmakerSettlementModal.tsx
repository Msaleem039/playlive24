...
export function BookmakerSettlementModal({ match, isOpen, onClose, onSettle }: BookmakerSettlementModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-3 sm:p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="bg-gradient-to-r from-green-600 to-green-700 text-white px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs sm:text-sm text-white/80 truncate">{match?.matchTitle || match?.homeTeam || "Match"}</p>
              {match?.selectedBet && (
                <p className="text-xs text-white/60 mt-1 truncate">Bet ID: {match.selectedBet.id}</p>
              )}
            </div>
          </div>
          <button onClick={onClose} className="text-white hover:bg-white/20 p-1.5 sm:p-2 rounded-lg transition-colors flex-shrink-0 ml-2">
            ...
            {match?.selectedBet ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
                ...
                <div className="sm:col-span-2">
                  ...
                  <span className="ml-2 font-mono text-xs break-all">{match.selectedBet.settlementId || "N/A"}</span>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
            ...
            <div className="space-y-3 sm:space-y-4">
              <div>
                ...
                <Input
                  ...
                  className="w-full text-sm sm:text-base border-gray-300 focus:border-[#00A66E] focus:ring-[#00A66E]"
                  required
                />
              </div>
        ...
        <div className="bg-gray-50 px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
          <Button
            onClick={onClose}
            className="bg-gray-400 hover:bg-gray-500 text-white px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg text-sm sm:text-base w-full sm:w-auto order-2 sm:order-1"
            disabled={isLoading}
          >
            Cancel
          </Button>
