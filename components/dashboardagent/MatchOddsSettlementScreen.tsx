...
import { RefreshCw, Play, Search, ChevronLeft, Zap } from "lucide-react"
import { Button } from "@/components/utils/button"
import { Input } from "@/components/input"
import { useGetPendingSettlementsQuery, useSettleMatchOddsMutation } from "@/app/services/Api"
import { toast } from "sonner"
import { MatchOddsSettlementModal } from "./MatchOddsSettlementModal"
...
export function MatchOddsSettlementScreen() {
  ...
  const matches = useMemo(() => {
    ...
    if (response.data && Array.isArray(response.data)) {
      return response.data.filter((match: any) => (match.matchOdds?.count || 0) > 0)
    }
    return Array.isArray(response) ? response.filter((match: any) => (match.matchOdds?.count || 0) > 0) : []
  }, [pendingData])
  ...
  if (selectedMatch) {
    const matchOddsBets = selectedMatch.matchOdds?.bets || []
    return (
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
            <h3 className="font-semibold text-lg">Match Odds Bets</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {matchOddsBets.length > 0 ? (
                  matchOddsBets.map((bet: any, index: number) => (
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center">
                      <p className="text-gray-500 font-medium">No match odds bets found</p>
                    </td>
                  </tr>
                )}
              </tbody>
    ...
    <div className="flex-1 overflow-y-auto p-6">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-gray-500 font-medium text-lg">No match odds matches found</p>
        </div>
      )}
    </div>
  )
