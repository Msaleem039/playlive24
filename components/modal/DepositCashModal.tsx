...
import { useTopupBalanceMutation } from "@/app/services/Api"
...
export default function DepositCashModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Deposit API call - userId:", userId, "amount:", amountNum, "remarks:", remarks)
      ...
      console.log("Deposit API success:", result)

      // 🔥 Update current user's balance instantly (optimistic update)
      // When depositing to a child, parent's balance decreases
      if (authUser) {
      ...
      // Show success toast
      toast.success(`Successfully deposited ${amountNum.toFixed(2)} to ${username}`, {
        description: remarks || "Deposit completed successfully"
      })
      ...
      const errorMessage = 
        ...
        error?.message || 
        "Failed to deposit cash. Please try again."
  ...
  const fullTitle = `Deposit Cash - ${username}`
